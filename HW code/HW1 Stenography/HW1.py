#Dan N
# HW 1:
#Dependent: numpy, opencv, and argparse
import numpy as np
import cv2
import argparse
import os
...
parser.add_argument("-d", "--debug", dest='DEBUG', help="Enable Debug", action='store_true')
parser.add_argument("-s", "--seed", dest='SEED', help="Seed Key")
parser.add_argument("-sa", "--shiftalg", dest='USEShiftAlg', help="Enable Shift Algorithm", action='store_true')
parser.add_argument("-w", "--writepic", dest='WritePic', help="Enable Writing of generated Pictures", action='store_true')
...
if args.USEShiftAlg:
	USEShiftAlg = args.USEShiftAlg
else:
	USEShiftAlg = False

if args.WritePic:
	WritePic = args.WritePic
else:
	WritePic = False
...
#Grab image characteristics
row = img.shape[0]
col = img.shape[1]

#Create the Frame arrays
